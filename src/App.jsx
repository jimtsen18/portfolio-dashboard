...
const KPICard = ({ label, value, value2, sub, color="#38bdf8" }) => (
  <div style={{ background:"#1a1f2e", border:"1px solid #2a3045", borderRadius:12, padding:"16px 20px", minWidth:0, height:"100%", boxSizing:"border-box" }}>
    ...
    <div style={{ fontSize:22, fontWeight:700, color, letterSpacing:"-0.5px", fontVariantNumeric:"tabular-nums" }}>{value}</div>
    {value2 && <div style={{ fontSize:22, fontWeight:700, color, letterSpacing:"-0.5px", marginTop:2, fontVariantNumeric:"tabular-nums" }}>{value2}</div>}
...
export default function App() {
  ...
  const filteredSnapshots = useMemo(() => {
    ...
    const latest = [...snapshots].reverse().find(s => s.usMarketValueUSD != null && s.usMarketValueUSD > 0);
    ...
    const usUSDRatio   = latest && latest.marketValue > 0 ? latest.usMarketValueUSD / latest.marketValue : null;
    const usUSDCostRatio = latest && latest.totalCost > 0 ? latest.usCostUSD / latest.totalCost : null;
    return filtered.map(s => ({
      ...
      usMarketValueUSD: (s.usMarketValueUSD != null) ? s.usMarketValueUSD : (usUSDRatio != null   ? Math.round(s.marketValue * usUSDRatio) : null),
      usCostUSD:        (s.usCostUSD != null)        ? s.usCostUSD        : (usUSDCostRatio != null ? Math.round(s.totalCost * usUSDCostRatio) : null),
    }));
...
`}</style>
      ...
      <div className="kpi-all" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:24 }}>
        <div className="kpi-order-1" style={{ height:"100%" }}><KPICard label="總市值"   value={"NTD$"+fmt(totalTWD)}  value2={"USD$"+fmt(totalTWD/usdTwd)}         sub={`投入成本 NT$${fmt(totalCost)}`}   color={CARD_COLORS.totalValue} /></div>
        ...
        <div className="kpi-order-4" style={{ height:"100%" }}><KPICard label={activePeriodLabel+" 已實現資本利得"}   value={"NT$"+fmtSign(Math.round(realizedInPeriod))}      sub="期間賣出交易累積"                    color={CARD_COLORS.realized}   /></div>
        <div className="kpi-order-5" style={{ height:"100%" }}><KPICard label={activePeriodLabel+" 已實現損益"} value={"NT$"+fmtSign(Math.round(realizedInPeriod + divIncome))} sub={"資本利得 NT$"+fmtSign(Math.round(realizedInPeriod))+" ・股息 NT$"+fmt(Math.round(divIncome))} color={CARD_COLORS.periodGain} /></div>
        ...
        <div className="chart-grid">
          ...
          <div style={{ background:"#1a1f2e", border:"1px solid #2a3045", borderRadius:12, padding:20, gridColumn:"1 / -1" }}>
            ...
            {(() => {
              ...
              const first = filteredSnapshots.length>=2 ? filteredSnapshots[0][key]||0 : 0;
              const last  = filteredSnapshots.length>=2 ? filteredSnapshots[filteredSnapshots.length-1][key]||0 : 0;
              ...
              return (
                <div style={{ marginTop:10, display:"flex", flexDirection: isMobile?"column":"row", justifyContent:"space-between", alignItems: isMobile?"flex-start":"flex-end", gap: isMobile?8:0 }}>
                  ...
                  <div style={{ display:"flex", gap:20 }}>
                    {chartView === "tw" && <>
                      <div>
                        <div style={{ color:"#6b7a99", fontSize:10, marginBottom:2 }}>台股最新市值</div>
                        <div style={{ color:"#f59e0b", fontWeight:700, fontSize:13 }}>NTD${fmt(Math.round(twMV))}</div>
                      </div>
                      <div>
                        <div style={{ color:"#6b7a99", fontSize:10, marginBottom:2 }}>台股持倉成本</div>
                        <div style={{ color:"#e11d48", fontWeight:700, fontSize:13 }}>NTD${fmt(Math.round(twCost))}</div>
                      </div>
                    </>}
                    {chartView === "us" && <>
                      <div>
                        <div style={{ color:"#6b7a99", fontSize:10, marginBottom:2 }}>美股最新市值</div>
                        <div style={{ color:"#34d399", fontWeight:700, fontSize:13 }}>USD${fmt(Math.round(usMV))}</div>
                      </div>
                      <div>
                        <div style={{ color:"#6b7a99", fontSize:10, marginBottom:2 }}>美股持倉成本</div>
                        <div style={{ color:"#f472b6", fontWeight:700, fontSize:13 }}>USD${fmt(Math.round(usCost))}</div>
                      </div>
                    </>}
                    {chartView === "total" && <div />}
                  </div>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:24, marginTop:8 }}>
              {marketPie.map((m,i) => (
                <div key={m.name} style={{ textAlign:"center" }}>
                  ...
                  <div style={{ color:i===0?"#38bdf8":"#a78bfa", fontWeight:700, fontSize:13, fontVariantNumeric:"tabular-nums" }}>USD${fmt(m.value/usdTwd)}</div>
                </div>
              ))}
            </div>
          </div>
                      ...
                      <tr key={p.symbol} style={{ borderTop:"1px solid #1e2535", background:i%2===0?"transparent":"#0b0f1c", transition:"background 0.1s" }}>
                        ...
                        <td style={{ padding:"11px 14px", textAlign:"right" }}>
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:2 }}>
                            <span style={{ color:"#e2e8f0", fontWeight:700, fontVariantNumeric:"tabular-nums" }}>NTD${fmt(p.valueInTWD)}</span>
                            <span style={{ color:"#6b7a99", fontSize:11, fontVariantNumeric:"tabular-nums" }}>USD${fmt(p.valueInTWD/usdTwd,2)}</span>
                            <div style={{ width:60, height:3, background:"#1e2535", borderRadius:2, marginTop:2 }}>
                ...
                {[...trades]
                  ...
                  return (
                    <tr key={t.id} style={{ borderTop:"1px solid #1e2535", background:i%2===0?"transparent":"#0f1422" }}>
                      ...
                      <td style={{ padding:"10px 14px", textAlign:"right", color:"#e2e8f0" }}>{t.market==="TW"?"NT$":"$"}{t.price!=null?(t.market==="US"?t.price.toFixed(6):t.price.toFixed(2)):"—"}</td>
